...
export function publicResearchSubmissionEnabled(env = process.env) {
  const value = String(
    env.AIRA_ALLOW_PUBLIC_RESEARCH_SUBMISSIONS ??
      env.ALLOW_PUBLIC_RESEARCH_SUBMISSIONS ??
      ''
  )
    .trim()
    .toLowerCase();
...
export async function submitResearchRecord(body = {}, env = process.env) {
  if (!publicResearchSubmissionEnabled(env)) {
...
export async function checkResearchConnection(env = process.env) {
  const backend = detectResearchBackend(env);
  const submissionEnabled = publicResearchSubmissionEnabled(env);
  if (!isValidBackend(backend)) {
    return {
      ...snapshotFor(backend),
      publicSubmissionEnabled: submissionEnabled,
      configured: false,
      ok: false,
      reachable: false,
      invalidBackend: true,
  ...
  if (backend === 'supabase') {
    ...
    return { ...snapshotFor(backend), publicSubmissionEnabled: submissionEnabled, ...snapshot };
  }
  if (backend === 'airtable') {
